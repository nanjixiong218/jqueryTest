	...
	return this.each(function(){ //return jQuery obj
		...
		$bubblewrap.mouseover(function(e){
			...
			if (e.target.tagName=="IMG"){
				...
				var od=imgspecs.od
				var nd=imgspecs.nd
				$img.parent().stop().css('zIndex', 1000).animate({left:ncoords[0], top:ncoords[1], width:nd.width+40, height:nd.height+40}, setting.speed).css({'border':'2px solid #AAAAAA','margin':'o auto'}) //animate image
		...
		$bubblewrap.mouseout(function(e){
			if (e.target.tagName=="IMG"){
				...
				var od=imgspecs.od
				var nd=imgspecs.nd
				$img.parent().stop().css('zIndex', 999).animate({left:0, top:0, width:od.width+40, height:od.height+40}, setting.speed).css({'border':'1px solid #E0E0E0','margin':'0 auto'}) //animate image
